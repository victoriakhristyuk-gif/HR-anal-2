...
### 3.2b. Когортный отчёт («Только сквозная когорта»)

Обычный детальный отчёт (не расширенная аналитика) можно построить только по сквозной когорте — сотрудникам, ответившим и в 2025, и в 2026 году. Режим включается флажком в Sidebar и доступен только при источнике `2026` (когорта всегда строится ОТ 2026 К 2025); при переключении источника на `2025` флажок сбрасывается автоматически. Включение когортного режима автоматически включает и блокирует флажок «Сравнить с 2025» — когортный отчёт всегда двухпериодный.

Логика выборки:

1. Пользовательские фильтры применяются раздельно к строкам `Ответы 2026` и `Ответы 2025` (`FilterEngine.applyFilters`) — так же, как в `AnalyticsService.build`.
2. Отфильтрованные строки обоих годов передаются в существующий `Cohort.build` (алгоритм сопоставления по нормализованному `Фамилия Имя` не дублируется и не переписывается).
3. Все показатели отчёта (размер выборки, eNPS, средние оценки, распределения, Топ-5, сравнение периодов, сырые данные) считаются только по согласованным массивам `matched.now`/`matched.before` — размеры выборок 2026 и 2025 в когортном отчёте всегда совпадают.
4. Если лист `Ответы 2025` не найден, в данных нет столбца `Фамилия Имя` или пересечение когорты пусто, отчёт не создаётся — Sidebar показывает понятную текстовую ошибку.
5. Когорта меньше 30 человек не блокируется, но в паспорт листа добавляется заметный жёлтый баннер-предупреждение о ненадёжности статистических выводов (тот же порог, что и в `Cohort.changes`).

Вывод состава когорты: перед разделом «Сырые данные» когортный отчёт содержит отдельный, свёрнутый по умолчанию раздел «Состав сквозной когорты» — таблица `ФИО` / `ФИО в 2025` (только если написание отличается) / `Статус сопоставления`. Раздел строится из `Cohort.roster` и содержит ровно тех, кто вошёл в когорту — без дублей ключа и неподписанных анкет (они уже исключены самим `Cohort.build`). ФИО нигде за пределами этого листа не появляются: не в Sidebar, не в `Сводная аналитика`, не в служебной сигнатуре отчёта/строки Summary и не в журнале выполнения (`Logger`/`console`).

**Приватность.** Раздел «Состав сквозной когорты» — персональные данные (полные ФИО реальных сотрудников). Доступ к листам когортных отчётов нужно ограничивать так же, как к любому другому источнику ПДн в этой таблице; при экспорте или пересылке отчёта раздел следует удалить или оставить свёрнутым только для получателей, которым ФИО действительно нужны.

Название листа без пользовательского имени: `Когортный отчёт` (без фильтров) или `Когорта • <значения фильтров>` — имя `Когорта` без дополнения не используется, оно занято листом расширенной аналитики (`Cohort` / `AnalyticsWriter`, см. 3.3). Сигнатура листа (`ReportBuilder.getReportKey_`) и ключ строки `Сводная аналитика` (`Summary.buildSampleKey_`) включают признак когорты только когда он `true` — поэтому когортный и обычный отчёт с одинаковыми фильтрами получают разные ключи и не заменяют друг друга, а ключи уже существующих обычных отчётов не меняются.

Источники: `src/Sidebar.html` (флажок `cohortOnly`, блокировка `compareWith2025`); `src/SidebarController.gs` — `buildReportFromSidebar`; `src/BatchReports.gs` — `run` (параметр `cohortOnly` пробрасывается и в пакетный режим — см. 3.2); `src/ReportService.gs` — `buildCohortReport_`, `computeReportMetrics_`; `src/Cohort.gs` — `build`, `roster`; `src/ReportBuilder.gs` — `renderCohortRoster_`, `generateCohortReportName`, `getReportKey_`; `src/Summary.gs` — `buildSampleKey_`; тесты — `src/CohortReportTest.gs`.
...
- `Связи срезов`;
- `Когорта`;
- `Сервисные vs доменные` — прямое сравнение «Сервисная команда» / «Доменная разработка» внутри Управления разработки ПО (`TeamTypeComparison.gs`, `AnalyticsWriter.writeTeamTypeComparison_`), строится независимо от фильтров, как и `Руководитель и команда`;
...
|---|---|---|
| A | `Фамилия Имя` | `text`; ключ когорты (`Cohort.KEY_QUESTION`); не фильтруется. Показывается как отдельная метрика ТОЛЬКО в разделе «Состав сквозной когорты» когортного отчёта (см. 3.2b) — сворачиваемый по умолчанию блок со списком ФИО участников. В сырых данных столбец, как и раньше, есть у любого отчёта. |
...
| T | Публичное имя `Work-life balance`, заголовок данных `График` | `scale4`: `да`, `скорее да`, `скорее нет`, `нет`. |
| U:Y | `Задачи`, `Ожидания`, `Проф мнение`, `Возможности роста`, `ОС от руководителя` | `scale4`: `да`, `скорее да`, `скорее нет`, `нет`. |
...
### 12.1a. Явка среди приглашённых

- **Назначение:** показать, какая доля приглашённых заполнила анкету в каждом году.
- **Алгоритм:** число строк-ответов выбранного года делится на число приглашённых того же года с листа `Численность`.
- **Периоды:** 2025 и 2026 рассчитываются раздельно; численность одного года никогда не используется как знаменатель другого.
- **Оргструктура:** переименования и переход отдела между управлениями связываются через стабильный `ID отдела`; название и управление хранятся отдельно для каждого года.
- **Необязательность:** пустой ID временно заменяется ключом из названия отдела. Если справочник заполнен частично или содержит ошибку, основные аналитические расчёты продолжаются, а численность и response rate не выводятся до исправления справочника.
- **Фильтры:** без фильтров используется численность компании; при фильтрах только по отделу/управлению/группе команд — численность выбранной оргструктуры (все три пересекаются через И). Для города, стажа, формата и ответов на вопросы процент не рассчитывается, потому что соответствующего знаменателя нет.
- **Контроль качества:** значение выше 100% помечает несогласованность числа ответов и справочника приглашённых.
- **Вывод:** приглашённые и явка показываются в паспорте отчёта, сводной аналитике, боковой панели и расширенных срезах по отделам/управлениям/группам команд; для расширенных срезов видны оба года.
- **Тип команды:** необязательная колонка листа `Численность` (`Год | ID отдела | Управление | Тип команды | Отдел | Численность`). Пустое значение — не ошибка, отдел просто не входит ни в одну группу по типу команды, но остаётся в компании/управлении/отделе. Логическая группа — составной ключ «Управление + Тип команды» (`Headcount.teamGroupOf`): одинаковое название типа в разных управлениях не объединяется. Производный фильтр/срез «Группа команд» использует этот ключ так же, как «Управление» использует само управление.

Источники: `src/Headcount.gs`, `src/ReportService.gs`, `src/Segments.gs`, `src/ReportBuilder.gs`, `src/Summary.gs`, `src/TeamTypeComparison.gs`.
...
- **Полный охват (`fullCoverage`):** только для срезов «Отдел»/«Управление»/«Группа команд», где известна численность (`Headcount`). Считается по точным целым `n === headcount`, а не по округлённому проценту явки. Типичный отдел компании — 10-12 человек: если ответили все, малое `n` означает не слабую выборку, а полное обследование команды — метрика при этом остаётся чувствительной к отдельным ответам, но вывод не понижается до «сигнал, а не факт» или «недостаточно данных» только из-за размера. Формулировка оговорки для одного года — `Segments.coverageCaveat(segment, previous)`. Для остальных срезов (город, стаж, формат работы и т.д.) численность не отслеживается, `fullCoverage` там не определяется, и `fragile` читается как обычная числовая осторожность без утверждений об охвате.
- **Надёжность текущего результата vs надёжность сравнения годов — раздельные проверки.** Подтверждённое отклонение сегмента от нормы компании считается по данным ТЕКУЩЕГО года (см. `METRICS`/`analyze`), поэтому его серьёзность/статус не должны зависеть от прошлогодней базы. `Segments.currentReliabilityLimited(segment)` смотрит только на текущий год (`coverageCaveat(segment, false).limitsReliability` или отсутствие валидной текущей eNPS-метрики) — её использует лист «Выводы» (`AnalyticsService.findings`, п.6) и колонка «Статус результата» листа «Отклонения срезов» (`AnalyticsWriter.writeSegments_`), поэтому обе поверхности не могут разойтись. `Segments.yearComparisonLimited(segment)` — отдельная, более широкая проверка (текущая база ограничена, ИЛИ прошлогодняя ограничена, ИЛИ одного из периодов нет), используется только для подписей рядом с годовой динамикой (`ReportBuilder.renderCompanyContext_`) и не влияет на статус текущего результата. Действующий расчёт `yearDelta.meaningful` (оба года `n ≥ FRAGILE_SEGMENT_SIZE`) не меняется ни одной из этих функций.
- **Срезы:** формат работы, стаж, город, отдел, управление, группа команд.
...
15. Поддерживать актуальность многолетнего листа `Численность`: проверять число приглашённых перед каждым циклом опроса и сохранять один стабильный ID отдела при переименовании. Response rate реализован отдельно от question-level coverage; неизвестный знаменатель не подменяется общей численностью.
...
- `Summary.COLUMNS.length === 43`, полная ширина со скрытым ключом — 44 столбца.
...
| Fragile | Сегмент с `n < 25` либо разрыв с числом критиков `< 20`, в зависимости от контекста. Про репрезентативность по явке ничего не говорит — см. Full coverage. |
| Full coverage | Только для «Отдел»/«Управление»/«Группа команд»: `n === headcount` (точное целое). Ответили все приглашённые — срез описывает всю команду, малое `n` не понижает надёжность вывода, даже если сегмент `fragile`. |
| Current vs comparison reliability | Две раздельные проверки: `currentReliabilityLimited` (только текущий год — влияет на статус результата и severity findings) и `yearComparisonLimited` (текущий ИЛИ прошлый год ограничен, ИЛИ периода нет — только для подписей рядом с годовой динамикой). Прошлогодняя явка/малая база не понижают статус текущего результата. |
...
- Когортный отчёт (3.2b) впервые выводит полные ФИО прямо на лист отчёта (раздел «Состав сквозной когорты») — это новая поверхность утечки ПДн при неаккуратном шаринге таблицы или экспорте листа; раздел свёрнут по умолчанию, но не защищён правами доступа отдельно от остального листа.
