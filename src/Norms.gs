...
const Norms = {
  ...
  /**
   * Явка (ответившие / штатная численность), ниже которой срез
   * "Отдел"/"Управление" помечается как малорепрезентативный по
   * охвату — отдельно от FRAGILE_SEGMENT_SIZE, который смотрит только
   * на абсолютное n и ничего не знает о численности (см. Headcount.gs).
   */
