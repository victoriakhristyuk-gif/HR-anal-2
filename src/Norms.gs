...
const Norms = {
  ...
  /**
   * Минимальный размер группы, при котором срез вообще показывается.
   * Ниже 10 человек любой показатель — это отдельные мнения, а не
   * характеристика группы; плюс возникает риск деанонимизации.
   */
  MIN_SEGMENT_SIZE: 10,

  /**
   * Размер группы, ниже которого показатель помечается как
   * «сигнал, а не факт». При n от 10 до 20 доверительный интервал
   * по eNPS составляет ±20–35 пунктов.
   */
